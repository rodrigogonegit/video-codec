...
class JpegLs(object):
	...
	def __init__(self, input_file_path):
		...
		logging.basicConfig(format=log_fmt_string, datefmt='%H:%M:%S', level=logging.INFO)
	...
	def encode_file(self):
		...
		self.output_file_stream = BitStream(self.__output_file_path, OpenMode.WRITE)

		# We want padding with zeros because of Golomb
		self.output_file_stream.set_padding_mode(True)

		# Open output file (encoded file)
		self.__output_golomb_obj = GolombEncoder(None, self.output_file_stream)

		print('Encoded file:', self.__output_file_path)

		# Write some header information
		self.output_file_stream.write_int(self.__yuv_file.frame_width, 4)
		self.output_file_stream.write_int(self.__yuv_file.frame_height, 4)
		type_dict = {'4:4:4': 0, '4:2:2': 1, '4:2:0': 2}
		self.output_file_stream.write_int(type_dict[self.__yuv_file.color_space], 4)
		self.output_file_stream.write_int(self.__yuv_file.number_of_frames, 4)
		self.output_file_stream.write_int(len(self.__yuv_file.raw_header), 4)
		self.output_file_stream.write_int(self.__m_param, 4)

		# Start of actual content. Write unmodified original header to encoded file.
		self.output_file_stream.write_bytes(self.__yuv_file.raw_header)
		...
		while True:
			...
			self.__logger.debug(f'{y.shape}, {u.shape}, {v.shape}')
			# print('\rProgress:', round(
			# 	counter / self.__yuv_file.number_of_frames * 100, 2), end='')
	...
	def decode_file(self):
		...
		print('# of Frames:\t', self.frame_count)
		print('Decoding. Output file:\t', self.__output_file_path)
		...
		for i in range(0, self.frame_count):
			self.__logger.debug(f'Processing frame number {i} of {self.frame_count} ')
			print(f'\rProgress: {round(i/self.frame_count*100.0, 2)} ', end="")
			self.__output_file_stream.write_str('FRAME\n')
			golomb_values = golomb_obj.decode(self.m_param, self.frame_width * self.frame_height, returnList=True)

			# Y plane
			self.decode_plane(self.frame_height, self.frame_width, golomb_values)

			# U plane
			golomb_values = golomb_obj.decode(self.m_param, uv_planes_cols * uv_planes_rows, returnList=True)
		...
		self.__logger.debug('Processed all frames.')
